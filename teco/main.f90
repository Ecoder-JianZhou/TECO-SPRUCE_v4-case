...
subroutine createNewCase()
    ...
    outdir = adjustl(trim(outdir))//"/"//adjustl(trim(simu_name))
    call CreateFolder(adjustl(trim(outdir)))
    outDir_nc     = adjustl(trim(outdir))//"/"//adjustl(trim(outDir_nc))
    outDir_csv    = adjustl(trim(outdir))//"/"//adjustl(trim(outDir_csv))
